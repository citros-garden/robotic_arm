  ...
  franka_msgs__msg__GraspEpsilon__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    franka_msgs__msg__GraspEpsilon * data =
      (franka_msgs__msg__GraspEpsilon *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!franka_msgs__msg__GraspEpsilon__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          franka_msgs__msg__GraspEpsilon__fini(&data[i]);
        }
        free(data);
        return false;
      }
